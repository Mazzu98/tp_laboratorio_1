
void printMenu(void);
int controller_leerArchivoBin(LinkedList*,char*);
int controller_altaCliente(LinkedList*,int*);
int controller_modificarCliente(LinkedList*);
int controller_bajaCliente(LinkedList*);
void controller_mostrarLista(LinkedList*);
int controller_eliminarLista(LinkedList*);
int controller_findById(LinkedList*,int);
int controller_sortCliente(LinkedList* );
int controller_insertarCliente(LinkedList*,int, eCliente*);
int controller_escribirBinario(LinkedList*,char*);
